        ...
        <header className="relative flex flex-col items-center gap-6">
          ...
          <div
            ...
            <div className="flex items-center gap-3 mt-1">
              <span
                className="relative inline-flex items-center gap-2 rounded-full border border-border/60 bg-muted/30 px-3 py-1 text-sm text-foreground">
                <span className="relative flex h-2.5 w-2.5">
                  <span
                    className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-60"></span>
                  <span
                    className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"></span>
                </span>
            ...
            <h2 className={"text-2xl text-muted-foreground"}>TypeScript
              Application
              Developer at <a
                href={"https://www.dvele.com/"}
                className="underline-offset-4 hover:underline">Dvele</a></h2>
            ...
            <div className="mt-2 flex items-center gap-5">
              <a href="https://www.instagram.com/frigon_e/" target="_blank"
                 rel="noreferrer noopener" aria-label="Instagram"
                 className="group inline-flex items-center justify-center">
                <svg
                  className="h-6 w-6 text-muted-foreground transition-colors group-hover:text-transparent"
              ...
              <a href="https://www.linkedin.com/in/ethan-frigon-aa6a031a0/"
                 target="_blank" rel="noreferrer noopener" aria-label="LinkedIn"
                 className="group inline-flex items-center justify-center">
                <svg
                  className="h-6 w-6 text-muted-foreground transition-colors group-hover:text-sky-500"
                  viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                  <path
                    d="M4.98 3.5C4.98 4.88 3.86 6 2.5 6S0 4.88 0 3.5 1.12 1 2.49 1s2.49 1.12 2.49 2.5zM.5 8.5h4V23h-4zM8.5 8.5h3.8v2h.05c.53-1 1.83-2.05 3.77-2.05 4.03 0 4.78 2.66 4.78 6.12V23h-4v-6.6c0-1.57-.03-3.6-2.2-3.6-2.2 0-2.54 1.72-2.54 3.5V23h-4V8.5z"/>
                </svg>
              </a>
              <a href="https://github.com/Frigon-e" target="_blank"
...
function AnimatedGroupCard({group, index}: {
  ...
  return (
    ...
    >
      <Card
        className="w-full bg-card border shadow-sm transition-transform duration-200 hover:-translate-y-1 hover:shadow-md hover:border-primary/30">
        <CardHeader>
          <h3
  ...
  return (
    <section className="flex flex-col items-center justify-center w-full">
      ...
      <div ref={ref}
        ...
        <Card
          className="w-[600px] max-w-[95vw] bg-card border shadow-sm transition-transform duration-200 hover:-translate-y-1 hover:shadow-md hover:border-primary/30">
        ...
        {workExperiences.map((experience, index) => {
          ...
          return (
            <div key={index} ref={ref}
              ...
              <Card
                className="bg-card border shadow-sm rounded-lg p-4 gap-1 transition-transform duration-200 hover:-translate-y-1 hover:shadow-md hover:border-primary/30">
                <h3
                  className="font-semibold text-primary">{experience.title}</h3>
                <p
                  className="text-muted-foreground text-sm">{experience.company}</p>
                <p
                  className="text-muted-foreground text-xs">{experience.duration}{experience.location ? ` • ${experience.location}` : ""}</p>
                {experience.summary ? (
                  <p
                    className="mt-3 text-sm text-muted-foreground">{experience.summary}</p>
                ) : null}
                {experience.achievements?.length ? (
                  <div className="mt-3">
                    <div
                      className="text-sm font-semibold text-foreground/90">Key
                      Achievements:
                    </div>
                    <ul
                      className="mt-1 list-disc pl-5 text-muted-foreground text-sm space-y-1">
                ...
                {experience.technologies?.length ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {experience.technologies.map((tech, i) => (
                      <span key={i}
                            className="inline-flex items-center rounded-md border border-border/60 bg-muted/20 px-2 py-1 text-xs text-foreground">{tech}</span>
                    ))}
                  </div>
                ) : null}
              </Card>
              ...
              <div className="w-1/2 flex justify-end pr-3 sm:pr-8">
                {isLeft && (
                  <div ref={ref}
                    ...
                    <Card
                      className="bg-card border shadow-sm rounded-lg p-4 text-left w-80 gap-1 transition-transform duration-200 hover:-translate-y-1 hover:shadow-md hover:border-primary/30">
                      <h3
                        className="font-semibold text-primary">{experience.title}</h3>
                      <p
                        className="text-muted-foreground text-sm">{experience.company}</p>
              ...
              <div className="w-1/2 flex justify-start pl-3 sm:pl-8">
                {!isLeft && (
                  <div ref={ref}
                       ...
                       style={{transitionDelay: delay}}>
                    <Card
                      className="bg-card border shadow-sm rounded-lg p-4 text-right w-80 gap-1 transition-transform duration-200 hover:-translate-y-1 hover:shadow-md hover:border-primary/30">
                      <h3
                        className="font-semibold text-primary">{experience.title}</h3>
                      <p
                        className="text-muted-foreground text-sm">{experience.company}</p>
